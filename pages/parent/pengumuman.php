...
$stmt = $db->query("SELECT p.*, u.nama as penulis FROM pengumuman p JOIN users u ON p.penulis_id=u.id WHERE FIND_IN_SET('parent', p.target_role) ORDER BY p.created_at DESC");
$list = $stmt->fetchAll();
...
<p class="page-subtitle mb-4">Informasi terbaru dari ustad dan admin</p>

<?php if (empty($list)): ?>
<div class="card"><div class="empty-state py-5"><i class="fas fa-bullhorn"></i><p>Tidak ada pengumuman.</p></div></div>
<?php else: ?>
<div class="row g-3">
